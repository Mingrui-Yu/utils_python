...
class PytorchKinematicsHelper:
    def __init__(
        ...
        """
        Args:
            pk_chain: pytorch_kinematics Chain object
            base_pose: Base pose [x, y, z, qw, qx, qy, qz] (7 elements)
            device: torch device
        """
        self.device = device

        # full chain
        self.chain = pk_chain.to(device=device, dtype=torch.float64)
        self.serial_chains = {}
        self.ik_solvers = {}
        self.ik_retry_counts = {}
        self.ik_initial_noise_stds = {}

        # base & world frame
        self.robot_joint_names = list(self.chain.get_joint_parameter_names())
        bpos = torch.tensor(base_pose[:3], device=device)
        bquat = torch.tensor(base_pose[3:7], device=device)
    ...
    def create_ik_solver(
        ...
        if ee_name not in self.serial_chains:
            raise KeyError(f"Serial chain for end effector '{ee_name}' has not been created.")

        serial_chain = self.serial_chains[ee_name]
        ik_dtype = serial_chain.dtype
        self.ik_retry_counts[ee_name] = num_retries
        self.ik_initial_noise_stds[ee_name] = (
            initial_noise_std.to(device=self.device, dtype=ik_dtype) if initial_noise_std is not None else None
        )

        if initial_noise_std is not None:
            assert initial_noise_std.shape[0] == len(self.robot_joint_names), (
                "initial_noise_std must match n_robot_joints"
            )

        joint_lims = torch.tensor(serial_chain.get_joint_limits(), device=self.device, dtype=ik_dtype)

        self.ik_solvers[ee_name] = pk.PseudoInverseIK(
            serial_chain,
            ...
            early_stopping_any_converged=early_stopping_any_converged,
            early_stopping_no_improvement=early_stopping_no_improvement,
            early_stopping_no_improvement_patience=early_stopping_no_improvement_patience,
            debug=debug,
            lr=lr,
            regularlization=regularlization,
        )
        self.ik_solvers[ee_name].joint_limits = self.ik_solvers[ee_name].joint_limits.to(
            device=self.device, dtype=ik_dtype
        )
        self.ik_solvers[ee_name].initial_config = self.ik_solvers[ee_name].initial_config.to(
            device=self.device, dtype=ik_dtype
        )
    ...
    def solve_ik_batch(
        ...
        if ee_name not in self.serial_chains:
            raise KeyError(f"Serial chain for end effector '{ee_name}' has not been created.")
        if ee_name not in self.ik_solvers:
            raise KeyError(f"IK solver for end effector '{ee_name}' has not been initialized.")

        serial_chain = self.serial_chains[ee_name]
        ik_solver = self.ik_solvers[ee_name]
        ik_dtype = serial_chain.dtype
        n_ik_retries = self.ik_retry_counts[ee_name]
        initial_noise_std = self.ik_initial_noise_stds[ee_name]
        matrix = matrix.to(device=self.device, dtype=ik_dtype)
        B = matrix.shape[0]
        ...
        if ref_configs is not None:
            ...
            ref_configs = ref_configs.to(device=self.device, dtype=ik_dtype)
            full_q = ref_configs.clone()
            serial_ref_configs = ref_configs[:, serial_indices]
            serial_ref_configs = serial_ref_configs.unsqueeze(1).repeat(1, n_ik_retries, 1)

            if use_ref_as_init:
                noise_std = (
                    initial_noise_std[serial_indices].view(1, 1, -1) if initial_noise_std is not None else 0
            ...
            full_q = torch.zeros((B, num_joints), dtype=ik_dtype, device=self.device)
            # re-sample initial configs (default: uniform sampling)
            ik_solver.initial_config = ik_solver.sample_configs(num_configs=n_ik_retries).to(
                device=self.device, dtype=ik_dtype
            )
        ...
        goal_tf_in_base = goal_tf_in_base.to(self.device, dtype=ik_dtype)
        sol = ik_solver.solve(goal_tf_in_base, ref_configs=noised_serial_ref_configs)
